function resi=NO2resi_lin(x0,meas,PAR)
% linear constrained minimization to retrieve NO2 in the 430-490 nm band
% meas are measurements
% PAR are additional parameters (e.g. o3, o4, coef and c0 TOA)
...
I_meas  = log(Idat) + no2_coef.*x0(:,1)+ o4_coef.*x0(:,2) + o3_coef.*x0(:,3) + x0(:,4)*ones(length(Xdat),1) + x0(:,5)*(Xdat) + x0(:,6)*(Xdat).^2 + x0(:,7)*(Xdat).^3;
