...
lbls=['No diff - 1 fiber','diff - 1 fiber','diff - 3 fiber',...
        'diff - 3 fiber','diff - 3 fiber','diff_2 - 16 fiber',...
 ...
 subplot(2,1,1);
 plot(vis_fova(1).deg,vis_fova(1).norm_raw(:,407));
 xlabel('Angle from Center');
 ...
 grid on;
 hold off;
 hl=legend('No diff - 1 fiber','diff - 1 fiber','diff - 3 fiber',...
        'diff - 3 fiber','diff - 3 fiber','diff_2 - 16 fiber',...
        'diff_2 - 16 fiber','diff_2 - 1 fiber','diff_2 - 1 fiber');
set(hl,'Position',[0.8 0.75 0.2 0.2],'Units','normalized');
 
 subplot(2,1,2);
 plot(nir_fova(1).deg,nir_fova(1).norm_raw(:,146));
 ylim([0.85 1.15]);
 xlabel('Angle from Center');
 ylabel('Normalized counts');
 ...
 for i=1:numv;
     ...
     vis_fovp(i).deg=abs(vis_fovp(i).AZ_deg(15:54))-mean(abs(vis_fovp(i).AZ_deg(15:54)));
     nir_fovp(i).deg=abs(nir_fovP(i).AZ_deg(15:54))-mean(abs(nir_fovp(i).AZ_deg(15:54)));
 ...
 set(fig, 'units', 'centimeter', 'position', [6 2 18 28]);
 subplot(2,1,1);
 plot(vis_fovp(1).deg,vis_fovp(1).norm_raw(:,407));
 xlabel('Angle from Center');
 ylabel('Normalized counts');
 ylim([0.85 1.15]);
 %set(gca,'Position',[],'Units','normalized');
...
set(hl,'Position',[0.8 0.75 0.2 0.2],'Units','normalized');
 ...
 subplot(2,1,2);
 plot(nir_fovp(1).deg,nir_fovp(1).norm_raw(:,146));
 ylim([0.85 1.15]);
 xlabel('Angle from Center');
 ylabel('Normalized counts');
