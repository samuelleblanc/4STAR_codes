function [flags,screened,good,figs] = visi_screen(time, tau, varargin)
% [flags,screen,good,figs] = visi_screen_v7(time, tau, varargin)
...
% 'nomask',flag_list: list strings matching flag fields that will not be
...
%   screen: bitwise mapping of flags fields into uint32.
...
if isstruct(flags)
    ...
    for f = length(flag_names):-1:1
        if isstruct(flags.(char(flag_names(f))))
            ...
            if size(flags.(char(flag_names(f))),2)==1
...
if isstruct(flags)&exist('flag_gray','var')
    ...
    for f = 1:length(flag_names)
        if any(strcmp(flag_gray,flag_names{f}))
            ...
            gray_bit=[gray_bit,f];%MK records bits where screen=1, screened_all_bits=0
        end
    end
end
...
%MK added 20131205 anything in goodnomask, if selected, overules all other
%flags and is not being screened out
set_screened_zero=zeros(size(screen));%MK
for i_bit=1:size(gray_bit,2)%MK
set_screened_zero(find(bitget(screen,gray_bit(i_bit))==1))=1;%MK
end%MK
if ~isempty(find(set_screened_zero))%MK
screened(logical(set_screened_zero))=0;%MK
end%MK
...
if isempty(figs)
    tau_fig = figure;zoom('on'); figs.tau_fig.h = tau_fig;
    leg_fig =figure;zoom('off'); figs.leg_fig.h = leg_fig;
    ...
    if nflags>1
        tau2_fig = figure;zoom('on'); figs.tau2_fig.h = tau2_fig;
        set(tau2_fig,'name','gray flags','units','normalized');
        figs.tau2_fig.pos = get(tau2_fig, 'position');
    end
else
    tau_fig = figs.tau_fig.h;   figure(tau_fig);
    leg_fig = figs.leg_fig.h; figure(leg_fig);
    ...
    if nflags>1
        tau2_fig = figs.tau2_fig.h; figure(tau2_fig);
...
for fld = 1:length(fields)
    subs = max([subs,sscanf(fields{fld},'panel_%d')]);
    if ~isempty(subs)
        ...
        for sb = length(subfields):-1:1
            if size(sub.(sprintf('panel_%d',subs)).(subfields{sb}),2)==1
...
if subs>0
    ...
    if exist('figs','var')&&isfield(figs,'aux_fig')
...
for bit = 1:nflags
    ...
    plot([t(1)],[-1*bit*NaN],syms(rem(bit,12)),'color',color(bit_c,:));
end
set(gca,'xtick',[],'ytick',[]);
...
for bit = 1:nflags
    ...
    test = logical(bitget(screen,bit_c));%MK modified 20140402
    plot([t(test),t(end)],real([tau(test),NaN]),syms(rem(bit,12)),'color',color(bit_c,:));
    grid on
end
...
%%
good = screened==0; %good if all flags in screened = 0
