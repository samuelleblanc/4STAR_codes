function [tau_o3 o3vcd tau_no2 no2vcd o4vcd MSEo3DU MSEno2DU] = o3no2corecalc(starsun,c0,cross_sections)
...
%--------------------------------
 ...
 ino2=find(wvis<=0.490&wvis>=0.430);ino2=ino2';
...
no2conv = load(fullfile(starpaths,'NO2_254K_convTech5.txt')); % to be converted back to [atm x cm] no2convQDOAS=load(fullfile(starpaths,'NO2_254K_convTech5.txt'));
...
O4xso3    = o4conv(io3,2);
NO2xso3   = (no2conv(io3,2));%no2conv(io3,2);
O3xso3    = o3conv(io3,2);
...
NO2xsno2   = (no2conv(ino2,2));%no2conv(ino2,2);
...
%
 ...
 for i=1:length(starsun.UTHh)
    ...
    if tau_aero_avg_sub(i,407) < 0 % 513 is 600 nm;407 is 500 nm
        % derive const range polynomial basis
        [p,S] = polyfit(wvis(nonabsidxconst==1),tau_aero_avg_mRay(i,nonabsidxconst==1),order);
        [y,delta] = polyval(p,wvis,S);
        poly_c(i,:)=p';
        poly(:,i)=y;
        polydelta(:,i)=delta;
...
%       plot(wvis(HCOHidx),polyHCOH_,'-r','linewidth',2);
%       legend('total OD (Rayleigh subtracted)','3rd order baseline fit');ylabel('OD');
%       axis([0.3 0.335 0 4]);title(datestr(starsun.tavg(i),'yyyy-mm-dd HH:MM:SS'));
%       ax(2)=subplot(212);
%       plot(wvis(HCOHidx),tau_aero_avg_mRay(i,HCOHidx)-polyHCOH_','-.c','linewidth',2);hold on;
%       plot(HCOHlambda(HCOHidx),0.6e19*HCOHxs(HCOHidx),'--r','linewidth',2);hold on;
%       plot(BrOlambda(HCOHidx),5e16*BrOxs(HCOHidx),'--','color',[0.8 0.5 0.5],'linewidth',2);hold on;
%       plot(Glyoxallambda(HCOHidx),0.6e19*Glyoxalxs(HCOHidx),'--','color',[0.2 0.7 0.9],'linewidth',2);hold on;
%       xlabel('wavelength [\mum]');ylabel('OD');
%       legend('OD after baseline subtraction','HCOH xs x 0.6E19','BrO xs x 1E16','Glyoxal xs x 0.6E19');
%       axis([0.3 0.335 0 0.3]);linkaxes(ax,'x');
%     
    %
...
no2spectrum_pca     = tau_aero_pca_avg_mRay(:,(ino2))-RRno2pcaavg' + ccoefno2pcaavg(6,:)'*basisno2(:,6)';% changes from 6 to 4 in basis vector
%no2spectrum         = tau_aero_avg_mRay(:,(ino2))-RRno2avg' + ccoefno2avg(4,:)'*basisno2(:,4)';
no2fit_pca          = ccoefno2pcaavg(6,:)'*basisno2(:,6)';  % pca filtered
...
% no2VCDavg    = real((((ccoefno2avg(4,:))*1000)./(starsun.m_NO2)')');
 no2VCDpcaavg = real((((ccoefno2pcaavg(6,:))*1000)./(starsun.m_NO2)')');

% create smooth no2 time-series
 xts = 60/3600;   %60 sec in decimal time
 [no2VCDavgsmooth, sn] = boxxfilt(starsun.UTHh, no2VCDpcaavg, xts);
...
if showfigure==1
     ...
     plot(wvis((io3)),o3residual(i,:),':k','linewidth',2);
     ...
     title(['O3 spectra and reconstruction errors ' datestr(starsun.t(i),'yyyy-mm-dd HH:MM:SS')]);
     ...
     for i=1:100:length(starsun.t)
         ...
         xlabel('wavelength [\mum]','fontsize',14,'fontweight','bold');title(strcat(datestr(starsun.t(i),'yyyy-mm-dd HH:MM:SS'),' no2VCD= ',num2str(no2vcd(i)),' RMSE = ',num2str(RMSEno2(i))),...
...
% save into mat file
 ...
 gas_matfile = strcat(gas.date,'_4stargases2baseline20140212','.mat');
