...
calibmodc0   = repmat(modc0',length(starsun.tavg),1);

sundist      = repmat(starsun.f(1),length(starsun.UTavg),length(starsun.w));

% rate using modc0
spc_modc0    = starsun.spc_avg./calibmodc0./sundist;
%

% tau_OD modc0
modc0Tw=spc_modc0.*exp((starsun.tau_ray_avg).*repmat(starsun.m_ray_avg,1,length(starsun.w)));
...
% spectrum calculated by modc0 calibration (vertical OD)
        [poly3_,poly3_c_,iter,order_lin,thresh,fn] = backcor(starsun.w(wln),tau_ODmodc0(i,wln),order_in2,thresh_in2,'atq');% backcor(wavelength,signal,order,threshold,function);
...
%% end of baseline subtraction 
 ...
 % these transmittances are used for AATS method CWV derivation
     Tw=spc_modc0(:,wln).*exp(tau_aero'.*(repmat(starsun.m_aero_avg,1,length(starsun.w(wln))))+...
...
%                        pause(0.0001);
   ...
   cwv_opt = (swv_opt(:,1)/H2O_conv);% retrieval is made on vertical./starsun.m_H2O_avg;  %conversion from slant path to vertical
   if wrange==4
