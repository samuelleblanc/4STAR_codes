function [NO2conc NO2resi no2OD tau_aero_subtract] = no2corecalc_wh2o(starsun,no2coef,o4coef,o3coef,h2ocoef,wln,tau_OD,nm_startpca,nm_endpca)
% retrieve no2 and derive fitted spectrum for subtraction
%-------------------------------------------------------
% MS: July, 26, 2014: NO2 fit (430-490 nm region)
% MS, 2015-04-15; this is modification of no2corecalc
%                 to retrieve no2 in the 480-515 nm
%                 region that includes water vapor
%-------------------------------------------------------
...
for i = 1:length(starsun.t)
    ...
    x0 = [0.3 1 0.3 0.5 0.05 -0.1 0.5]; % this is initial guess (-2 for 6th element -0.1 for 7th element)
     if size(tau_OD,2)<1556 && nargin > 7
    ...
    PAR  = [no2coef(wln)/1000 o4coef(wln)*10000 o3coef(wln) h2ocoef(wln)*10000];
       % Set Options
       
       options = optimset('Algorithm','sqp','LargeScale','off','TolFun',1e-6,'Display','notify-detailed','TolX',1e-6,'MaxFunEvals',1000);%optimset('Algorithm','sqp','TolFun',1e-12);%optimset('MaxIter', 400);
       ...
       if ~isNaN(y(1)) && isreal(y) && sum(ypos)>length(wln)-15 && sum(isinf(y))==0
            ylarge = logical(y>=5);
            if sum(ylarge)<10
            [U_,fval,exitflag,output]  = fmincon('NO2resi_wh2o',x0,[],[],[],[],lb,ub, [], options, meas,PAR);
            end
             ...
             sc_residual = [sc_residual;real(fval)];
               ...
               % plot fitted figure
                   ...
                   yopt_ =  exp(-(((no2coef(wln))/1000).*real(U_(1)))).*exp(-(o4coef(wln).*10000*real(U_(2)))).*exp(-(o3coef(wln).*real(U_(3))))...
                            .*exp(-(h2ocoef(wln).*10000.*real(U_(4)))).*exp(-(U_(4) + U_(5)*meas(:,1) + U_(6)*meas(:,1).^2));
                   yopt  = -log(yopt_);
                   yno2subtractall   =  -log(exp(-((no2coef(wln)/1000).*real(U_(1)))).*exp(-(o4coef(wln).*10000*real(U_(2)))).*exp(-(o3coef(wln).*real(U_(3)))).*...
                                             exp(-(h2ocoef(wln).*10000.*real(U_(4)))));
                   ...
                   yh2osubtract  =  -log(exp(-(h2ocoef(wln).*10000.*real(U_(4)))));
               ...
               % assign fitted spectrum to subtract
                ...
                if ~isNaN(U_(1))
                         ...
                         plot(starsun.w(wln),yh2osubtract,'--y');hold on;
