function [tau_aero_fitsubtract tau_aero_specsubtract gas] = gasescorecalc(starsun,vislampc0,nirlampc0,model_atmosphere)
...
% h2o
 ...
 nm_0675 = interp1(wvis,[1:length(wvis)],0.675,  'nearest');
...
tau_aero_fitsubtract      = tau_ODslant;
%
%-------------------------------------
...
for i = 1:length(starsun.t)
       ...
       if wrange==3
           x0 = [real(nanmean(U(i,:))) 50000];
       elseif wrange==4
           x0 = [real(nanmean(U(i,:))) 100];
       elseif wrange==5
           x0 = [real(nanmean(U(i,:))) 1000 1000 1000];
       else
           x0 = real(nanmean(U(i,:)));
       end
       if lampcalib==1
       ...
       if ~isNaN(y(1)) && isreal(y) && sum(ypos)>length(wln)-15
           ...
           elseif wrange==5
            ...
            if isNaN(U_(1)) || ~isreal(U_(1)) || U_(1)<0
               ...
               % plot fitted figure
               if wrange==3
                   yopt_ =  exp(-afit_H2O(wln,kz).*(real(U_(1))).^bfit_H2O(wln,kz)).*exp(-(o2coef(wln).*real(U_(2))));
                   yopt  = -log(yopt_);
               elseif wrange==4
...
%                     pause(0.0001);
               ...
               tau_aero_fitsubtract(i,wln) = tau_ODslant(i,wln) - wvODfit(i,wln) + baseline(i,:);
            end
       else
           if wrange==3||wrange==4
               U_ = [NaN NaN];
...
% spectrum subtraction by spectral measured (not amount retrieved).
spec_subtract = wvOD;

%% subtract/retrieve CO2
   wln = wln_nir3;
   [CH4conc CO2conc CO2resi co2OD co2subtract co2spec] = co2corecalc(starsun,ch4coef,co2coef,wln,tau_ODslant);
   ...
   tau_aero_fitsubtract(:,wln_nir3) = co2subtract(:,wln_nir3);
...
for i=2
   ...
   [O2conc O2resi o2OD o2subtract o2spec] = o2corecalc(starsun,o2coef,wln,tau_ODslant);
   if i==1
...
%% subtract/retrieve O3/h2o/o4 region
   wln = wln_vis6;
   [O3conc H2Oconc O4conc O3resi o3OD o3subtract] = o3corecalc(starsun,o3coef,o4coef,h2ocoef,wln,tau_ODslant);
   ...
   tau_aero_fitsubtract(:,wln_vis6) = o3subtract(:,wln_vis6);
   spec_subtract(:,wln_vis6)        = o3subtract(:,wln_vis6);
%%
%%
% save wv retrieval
gas.cwv940     = cwv.cwv940;    % this is from fit
gas.cwv940avg  = avg_U1;        % this is from coef avg procedure
gas.cwv940std  = std_U1;        % this is from coef avg procedure
%%
%% calculate wv based on 940 band
% swv_ =  exp(-(h2ocoef*real(avg_U1'*H2O_conv)));
% swv  = -log(swv_);
%figure;plot(starsun.w,swv,'-g');legend('cwv calc 940 nm');
%%
%%
...
tau_aero_specsubtract = tau_ODslant - spec_subtract;
...
figure;plot(starsun.w(wln_nir2),tau_ODslant([1000],wln_nir2),'-b');hold on;plot(starsun.w(wln_nir2),wvODfit([1000],wln_nir2),'--g');hold on;
...
figure;plot(starsun.w,tau_ODslant([end-500],:),'-b');hold on;plot(starsun.w,wvODfit([end-500],:),'--g');hold on;
       plot(starsun.w,tau_aero_fitsubtract(end-500,:),'-r');hold on;plot(starsun.w,co2OD([end-500],:),'--g');hold on;
...
% plot two subtraction methods:
 figure;
 ax(1)=subplot(211);
 plot(starsun.w,tau_ODslant(end-500,:),'-r');hold on;plot(starsun.w,tau_aero_specsubtract(end-500,:),'--b');
 ylabel('OD');legend('total OD (Rayleigh subtracted)','aerosol OD (Rayleigh and water OD subtracted)');
 ax(2)=subplot(212);
 plot(starsun.w,tau_ODslant(end-500,:),'-r');hold on;plot(starsun.w,tau_aero_fitsubtract(end-500,:),'--b');
 xlabel('wavelength');
