function [tau_aero_woH2OCO2 tau_aero_woH2OCO2fit cwv940 cwvopt1100 QA avg_U1] = cwvcorecalc(starsun,modc0,vislampc0,nirlampc0,model_atmosphere,cross_sections)
...
pp    = length(starsun.t);
%
caliblampvis = repmat(vislampc0(:,3)',length(starsun.tavg),1);
caliblampnir = repmat(nirlampc0(:,3)',length(starsun.tavg),1);
caliblamp    = repmat([vislampc0(:,3)' nirlampc0(:,3)'],length(starsun.tavg),1);
...
vis.ODlamp  =-log(vis.ratelamp./repmat(vislampc0(:,3)',pp,1))./repmat(starsun.m_aero,1,qqvis);
...
%% subtract/retrieve CO2
   wln = wln_nir3;
   [CH4conc CO2conc CO2resi co2OD] = co2corecalc(starsun,ch4coef,co2coef,wln,tau_ODslant);
   cwv.band1600co2 = CO2conc./starsun.m_ray_avg;% slant converted to vertical
   cwv.band1600ch4 = CH4conc./starsun.m_ray_avg;% slant converted to vertical
   cwv.band1600resi= CO2resi;
%%
%% derive QA flags
[cwv.QA cwv.watervapor800_1100] = watervaporQA(cwv);
QA     = cwv.QA;
cwv940     = cwv.cwv940;    % this is from fit
%cwvopt800_1100 = cwv.watervapor800_1100;
cwvopt1100 = cwv.cwv1100;
%%
...
%
tau_aero_woH2OCO2 = tau_ODslant - wvOD - co2OD;
% subtraction with fitted OD
tau_aero_woH2OCO2fit = tau_ODslant - wvODfit - co2OD;
% plot two subtraction methods:
 figure;
 ax(1)=subplot(211);
 plot(starsun.w,tau_ODslant(13000,:),'-r');hold on;plot(starsun.w,tau_aero_woH2OCO2(13000,:),'--b');
 ylabel('OD');legend('total OD (Rayleigh subtracted)','aerosol OD (Rayleigh and water OD subtracted)');
 ax(2)=subplot(212);
 plot(starsun.w,tau_ODslant(13000,:),'-r');hold on;plot(starsun.w,tau_aero_woH2OCO2fit(13000,:),'--b');
 xlabel('wavelength');
