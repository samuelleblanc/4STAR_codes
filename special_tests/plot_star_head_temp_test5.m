...
dir = 'C:\Users\sleblan2\Research\4STAR\roof\20150325\';
daystr = '20150325';
...
tests(1).label = 'Upper-Y flight fiber #1';
tests(1).icool = [117:456]; %from 19:46 to 19:52
tests(1).iheat = [479:1588]; %from 19:52 to 20:21
tests(1).idark = [108];  %19:52
tests(1).ylim  = [97.5,103.5];

tests(2).label = 'Upper-Y flight fiber #2';
tests(2).icool = [1633:1939]; %from 20:26 to 20:44
tests(2).iheat = [1958:3351]; %from 20:46 to 21:24
tests(2).idark = [1950]; % 20:25
tests(2).ylim  = [98.5,102.5];

tests(3).label = 'Upper-Y flight fiber + diffuser';
tests(3).icool = [4025:4045]; %from 21:24 to 21:44
tests(3).iheat = [4053:4357]; %from 21:50 to 22:36, missing data points because of small spectro issue
tests(3).idark = [4048]; %21:24
