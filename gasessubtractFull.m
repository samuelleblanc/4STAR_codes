...
for k=1:length(starsun.t)
   ...
   plot(starsun.w,tau_OD_fitsubtract5(k,:),'--m','linewidth',2);hold on;%no2
   ...
   xlabel('wavelength','fontsize',12);ylabel('OD','fontsize',12);
                       title([datestr(starsun.t(i),'yyyy-mm-dd HH:MM:SS') ' Alt= ' num2str(starsun.Altavg(i)) 'm']);
                       ymax = yopt + 0.2;
                       if max(ymax)<0 
                           ymaxax=1; 
                       else
