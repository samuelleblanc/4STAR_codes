function [aod, min_rms, fit_rms, aod_pfit, aod_afit, aod_xfit] = tau_xfit(ws,tau, block)
% function [aod, min_rms, fit_rms, aod_pfit, aod_afit, aod_xfit] = tau_xfit(ws,tau, block);
...
toc
fit_rms = [rms_2,aod_afit_rms, aod_xfit_rms]; 
[min_rms,m_ij] = min(fit_rms,[],2);
r_1 = m_ij==1; r_2 = m_ij==2; r_3 = m_ij==3;
aod_ = aod_xfit;
aod_(r_2,:) = aod_afit(r_2,:);
