function [mark, mrg_out] = korus_clean_dirty_dtau
...
mrg_name = getfullname(['korusaq-mrg*',datestr(star.t(1),'yyyymmdd'),'*.ict'],'mrg');
mrg = rd_ict(mrg_name);
flight_str = ['KORUS-AQ ',datestr(mrg.time(1),'yyyy-mm-dd')];
...
[face_angle, area_norm]  = bluff_face(90-star.El_deg, star.AZstep./(-50));
...
while ~done
    ...
    figure_(2);
    plot(serial2hs(mrg.time), 1000.*mrg.ALTP,'r.',serial2hs(mrg.time), 1000.*mrg.ALTP+cina_,'-o');% dynamicDateTicks
    ...
    while ~out                
        ...
        elseif mn ==4
            ...
            try
                ...
                disp('BAD VALUE ENTERED. Try again.');
                sf = input('Input a new scale factor to apply to the last adjustment: ');
            end
            if length(sf)>1
                sf  = sf(end);
...
recolor(these, (mrg_out.time(yinx(2:end))-mrg_out.time(1)).*24); 
xlabel('wavelength'); ylabel('dOD'); title({flight_str; 'Accumulated AOD error vs wavelength'});
cb = colorbar; set(get(cb,'title'),'string','hour in flight','rotation',-90);
