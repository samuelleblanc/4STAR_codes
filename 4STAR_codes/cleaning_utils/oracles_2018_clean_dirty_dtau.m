function [mark, mrg_out] = oracles_2018_clean_dirty_dtau
...
% Modified: SL, v1.0, 2017/04/04, migrated from korus_clean_dirty_dtau.m to oracles
% Modified: SL, v1.1, 2018/05/06,  

version_set('v1.1')

in_star_file = getfullname('*2018*starsun*.mat','starsun');
...
[ii,dt] = knnsearch(star.t,mrg.time);
idd = dt<1.0/3600.0/24.0; % Distance no greater than one second.
for wii = length(wls):-1:1
    iww = interp1(star.w, [1:length(star.w)], wls(wii)./1000, 'nearest');
    star.(sprintf('i_%3.0f',wls(wii))) = iww;
    %AODs(:,wii) = mrg.(sprintf('AOD_%3.0fnm_4STAR',wls(wii)));
    %AODs(:,wii) = mrg.AOD(wii,:);
    AODs(:,wii) = NaN([length(mrg.time),1]);
    AODs(idd,wii) = star.tau_aero(ii(idd),iww);
end
...
try
    ...
    fla = load(in_star_file, 'flags');
    qual_flag = fla.flags.bad_aod;
    mrg.AOD_QualFlag_4STAR = zeros(size(mrg.time));
    mrg.AOD_QualFlag_4STAR(cina) = qual_flag(ainc);
    ainc(isNaN(sum(AODs(cina,wii),2)) | mrg.AOD_QualFlag_4STAR(cina)~=0) = [];
    cina(isNaN(sum(AODs(cina,wii),2)) | mrg.AOD_QualFlag_4STAR(cina)~=0) = [];
    cina_ = NaN(size(mrg.time)); cina_(cina) = 0;
    ainc_ = NaN(size(star.t)); ainc_(ainc) = 0;
...
try
    mrg.TSI1_Scat_Tot(2,mrg.TSI1_Scat_Tot(2,:)<-100) = NaN;
catch
   disp('Problem with the scattering data') 
end
...
elseif isfield (mrg, 'CDP_LWC');
    cld = zeros(size(mrg.CDP_LWC)); cld(mrg.CDP_LWC<0.05)=NaN;
else
...
try;
    disp('Some daods needed to be sanitized')
mrg_out.dAODs(mrg_out.dAODS<0.0) = 0.0;
catch
    disp('No aods needed to be sanitized')
end;
mrg_out.wl_nm = wls;
