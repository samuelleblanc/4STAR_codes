...
for i=idx_file_proc
    ...
    for it=1:length(Start_UTCs)
       [dt,ii] = min(abs(t2utch(flag.time.t)-UTC(it)));
       if dt<1.0
          data.qual_flag(it) = qual_flag(ii); 
       end
    end
    %    [tutc_unique,itutc_unique] = unique(tutc);
%    data.qual_flag = interp1(tutc_unique,double(qual_flag),UTC,'nearest');
%    data.qual_flag = qual_flag;
