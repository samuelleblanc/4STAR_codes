function [flags,screened,good,figs] = visi_screen_v14(time, tau, varargin)
...
while isempty(find([1,2,3],time_choice))
   time_choice = menu('How should time be displayed?','Dynamically yyyy/mm/dd or HH:MM:SS','HH.hh','day of year');
end
switch time_choice
   case 1
      %         t = serial2hs(time);
...
% zoom('on');
extra_buttons = {'FLAGGING MODE','SELECTION MODE','semilog ON/OFF','IMPORT FLAGS','Add/Remove FLAG','UNDO','REDO','CLEAR','RESET','DONE','ABORT'};
...
while ~done
   ...
   elseif button==menu_len-8 % toggle semilogy
      if strcmp(get((back_ax),'yscale'),'linear')
         logy(back_ax);
      else
         liny(back_ax);
      end
   elseif button==menu_len-7 % import flags
      ...
      flags = merge_flags(flags, in_flags);
      if isstruct(flags)
         flag_names = fieldnames(flags);
         for f = length(flag_names):-1:1
            if isstruct(flags.(char(flag_names(f))))
      ...
      % nflags = length(fieldnames(flags));
      nflags = length(flag_names); new_gray = flags.flag_struct.flag_noted;
      manual_flags.nomask_list = unique([flag_gray,new_gray],'stable');
      if isstruct(flags)&exist('flag_gray','var')
         %     flag_names = fieldnames(flags);
         for f = 1:length(flag_names)
            if any(strcmp(flag_gray,flag_names{f}))
      ...
      for bit = 1:nflags
   screen = bitor(screen, bit, flags.(char(flag_names(bit)))>0);
      end
   elseif button== menu_len-6 % add/remove flag
