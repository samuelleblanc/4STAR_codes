function [aod_fit, fit_rms, Ks,log_nmodes] = fit_aod_nbasis(wl, aod, wl_out);
% [aod_fit, fit_rms, Ks,log_nmodes] = fit_aod_nbasis(wl, aod,wl_out);
% wl should be in nm and Mx1 or 1xM
% aod should be Mx1 or 1xM or MxN with M matchin wl
% wl_out should be Nx1 or 1xN
% Fits log of supplied AOD as a combination of log_aod normalized basis vectors at selected
...
% 2020-11-26: v1.1, Connor, testing with basis normalized to unity
version_set('1.1');
...
if size(wl,2)==1 && all(size(wl)==size(aod))
    ...
    aod_mode = load([strrep(which('fit_aod_nbasis.m'),'fit_aod_nbasis.m','aod_SD_nmode.mat')]);
...
%     sub_modes = sub_modes./(ones(size(sub_modes,1),1)*max(sub_modes));
    ...
    % Perhaps a better approach would be to exclude coefficients based on 
    % contributing less than x% of the total at all wavelengths.
