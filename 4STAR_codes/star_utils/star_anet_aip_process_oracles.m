function anetaip = star_anet_aip_process_oracles(s)
...
if ~exist('s','var')
    s = getfullname('*.mat','starsky','Select star sky mat file.');
end
if strfind(s,'.mat')
    if exist(s,'file')
        [pname,fname, ext] = fileparts(s);
        s = load(s);
        if isfield(s,'filename');
            s.filename = [pname, filesep, fname, ext];
        end
    end
else
    s = starsky(s);
...
[pname_mat,~,~] = fileparts(s.filename);
...
if ~isfield(s,'Az_gnd')
    ...
    if sum(s.Alt<0)>5 % Then we're probably on the ground so determine mechanical offsets while tracking
