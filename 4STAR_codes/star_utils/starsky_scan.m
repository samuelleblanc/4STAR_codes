...
% Generate sky scans from starsky struct.  It applies Euler-angle rotation
% before computing scattering angles and offsets yielding symmetric near-sun
% sky radiances. The CW and CCW branches of ALM are assessed independently.
% If offsets are too high, or too few valid points were available due to
% saturation then the sky is deemed "suspect".  In the case of ALM scans
% offsets are determined for the pos and neg portions of the CW branch, and also
...
% less duplicates pre-existing behaviour but with syntax based on the AML
% treatment. 
% 
...
%       hold('off')
      ...
      star.good_ppl = good_ppl;
      star.good_sky = good_ppl;
   else
      %%
      good_almA = CCW&(POS|NEG);
...
%       star.good_alm = good_almA|good_almB;
      ...
      star.good_almA = CCW&(POS|NEG);
      star.good_almB = CW&(POS|NEG);
      star.good_alm = good_almA|good_almB;
