function spc = rd_spc_raw_F4(infile);
...
if ~exist('infile','var')||~exist(infile,'file')
   infile = getfullname_('*NIR_FOV*.dat','4STAR_F4','Select spectrometer file (NIR or VIS)');
end
% %Mission: 
% %Operator: 
% %Detector Type: NIR
...
% figure; ax(1) = subplot(3,1,1); plot([1:length(spc.AZstep)],spc.AZstep,'o-'); legend('AZstep');
% title(spc.fname, 'interp','none')
% ax(2) = subplot(3,1,2);  plot([1:length(spc.AZstep)],abs(spc.AZ_deg),[1:length(spc.AZstep)],-spc.AZstep./50,'o-'); 
% lg = legend('AZ_deg','-AZstep/50'); set(lg,'interp','none')
% 
% ax(3) = subplot(3,1,3);  plot([1:length(spc.AZstep)],abs(spc.AZ_deg)+spc.AZstep./50,'o-'); legend('difference')
% linkaxes(ax,'x')
% if any((abs(spc.AZ_deg)+spc.AZstep./50)~=0)
%     disp('What happened?');
% end
% close(gcf);
