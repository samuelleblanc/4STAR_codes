...
% lamp spectra time series in color plots - take long
plotlampspectra=false;
if plotlampspectra
    figure; % against time; no rotation only
    yyref=nanmean(rate1(idx(1,1):idx(1,2),:));
    for i=1:1;
        if abs(rotmean(i))<=minrotation
            xx0=AZstep(idx(i,1):idx(i,2))/(-50); % AZ angle
            if any(xx0)
            ...
            idx1=idx(i,1):1:idx(i,2);
            yy=rate1(idx1,:);
            %         semilogx(w, yy, '-', 'linewidth', 2);
            yysm=boxxfilt(t(idx1), yy, 10/86400);
            scattery(repmat(w,size(yy,1),1), 'Wavelength',...
                yy./repmat(yyref,size(yy,1),1), ['Count Rate, avg(' datestr(t(idx(1,1)),13) '-' datestr(t(idx(1,2)),13) ')=1'], ...
                ones(size(yy))*24, '', ...
                repmat(t(idx1),size(w)), 'UTC');
            hold on;
        end;
    end;
    clear idx1;
