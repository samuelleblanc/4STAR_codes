...
if star.isALM && ~isempty(almA_ii)&&~isempty(almB_ii)
    ...
    rad_A = star.skyrad(almA_ii(inA),star.aeronetcols(wl_test_ii));
    rad_B = star.skyrad(almB_ii(inB),star.aeronetcols(wl_test_ii));
    rad_sym = abs((rad_A - rad_B)./((rad_A + rad_B)./2));
    
    % AERONET requires <20% except for Az=180 which is <5%
    sym = ~any(rad_sym>0.20,2);
    sym(az_180) = ~any(rad_sym(az_180,:)>0.05,2);
    ...
    ss(2)=subplot(3,1,2);
    plot(SA_A,100.*rad_sym,'-o',SA_A+bad,100.*rad_sym,'rx');
