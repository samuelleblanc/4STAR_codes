...
tests.tau_test = 1.5; tests.sky_test = 1.0;
...
if star.isALM
    ...
    if isempty(almA_ii) || isempty(almB_ii)
        ...
        % AERONET requires <20% except for Az=180 which is <5%
        % Uncommenting to line 75 on 2020-07-24 to enforce symmetry test
...
% 
        ...
        if (tests.N_SA_lt_6>=2) & (tests.N_SA_lt_30>=5) &  (tests.N_SA_lt_80>=4) &  ...
                (tests.N_SA_gte_80>=3) & (tests.N_SA >= 21) &  max(tests.sel_max)<40;
            tests.sky_test = 2.0;
        end         
        % Solar El must be < 40 to ensure large enough range of scattering angle in
        % ALM scan
        if tests.tau_440 > 0.4 
            tests.tau_test = 2;
        end          
        if tests.sky_test>=2 & tests.tau_440 > 0.4 
            lv = 2; 
        end        
    end
elseif star.isPPL
