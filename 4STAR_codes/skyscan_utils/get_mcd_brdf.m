...
if length(mcd_hdf)~=1 || ~isafile(char(mcd_hdf))
   web_str2 = ['https://ladsweb.modaps.eosdis.nasa.gov/archive/allData/6/MCD43A1/' yyyy '/' doy '.json'];
   blop2 = webread(web_str2);
   index2 = find(cellfun(@(x) any(strfind(x,mcd_str(1:end-5))),{blop2.name})==1)
   mcd_filename = blop2(index2).name;
   disp(['*** Please download the MODIS BRDF file at: ' 'https://ladsweb.modaps.eosdis.nasa.gov/archive/allData/6/MCD43A1/' yyyy '/' doy '/' mcd_filename])
