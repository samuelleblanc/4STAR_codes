...
if isavar('outname')&&isafile(outname)
    ...
    if isafile(inputname) % Read a bunch of supporting data from the input file
        ...
        lv_i = findstr(anetaip.output_fname,'.output'); lv_in = sscanf(anetaip.output_fname(lv_i+[-2 -1]),'%f')./10;
        sky_test = lv_in; tau_test = lv_in; %Default values for sky_test and tau_test
        fid = fopen(anetaip.input_fname);
        done = false;
        inline = [];n= 0;
        while isempty(strfind(inline,'% Extras'))&& ~feof(fid)
            last_line = ftell(fid);
            inline = fgetl(fid);
            if ~isempty(strfind(inline,'albedo(IW)'))
                n= n +1;
                sfc_alb(n) = sscanf(inline,'%f %*[^\n]');
            end
        ...
        Extra = ftell(fid); 
        fseek(fid, last_line,-1);
        Extra_text = char(fread(fid))'; 
        if isempty(strfind(outfile,'% Extras'))
            outfile = [outfile,Extra_text];
        ...
        fseek(fid, Extra,-1);
        inline = fgetl(fid);
        while ~strcmp(inline(1),'%')&&~feof(fid)
            [field,vals] = strtok(inline,'=');
            field = deblank(field); vals = strrep(vals(2:end),',',' ');
            extras.(field) = sscanf(vals,'%f');
            inline = fgetl(fid);
        end
        labels = textscan(inline(2:end),'%s','Delimiter',[","," "],'MultipleDelimsAsOne',1);
        labels = labels{1};
        pixel_block = textscan(fid, '%d %f %f %f %f %*[^\n]','Delimiter',[","," "]);
        for L = 1:length(labels)
            extras.fit_details.(labels{L}) = pixel_block{L};
        end
        anetaip.input.extras = extras;
...
%     end
    ...
    fraction = sscanf(inline,'%f'); land_fraction = fraction(1);sea_fraction = fraction(2);
