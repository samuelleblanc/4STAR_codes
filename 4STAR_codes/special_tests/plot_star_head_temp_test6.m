...
% Written (v1.0): Samuel LeBlanc, NASA Ames, March 27th, 2015
% Modified:
%
% -------------------------------------------------------------------------
%function plot_star_fiber_temp_test6
%version_set('v1.0')
clear;
toggle.docontour = false;
dir = 'C:\Users\sleblan2\Research\4STAR\roof\20150330\';
daystr = '20150330';
...
tests(1).label = 'Upper-Y flight fiber with diffuser test #1';
tests(1).icool = [801:1082]; %from 19:46 to 19:52
tests(1).iheat = [1684:2466]; %from 19:52 to 20:21
tests(1).idark = [1666];  %19:52
tests(1).ylim  = [97.5,103.5];

tests(2).label = 'Upper-Y flight fiber with diffuser, no contact';
tests(2).icool = [2617:2905]; %from 20:26 to 20:44
tests(2).iheat = [2929:3064]; %from 20:46 to 21:24
tests(2).idark = [2587]; % 20:25
tests(2).ylim  = [98.5,102.5];

tests(3).label = 'Upper-Y flight fiber with diffuser test #2';
tests(3).icool = [3186:3354]; %from 21:24 to 21:44
tests(3).iheat = [3495:4159]; %from 21:50 to 22:36, missing data points because of small spectro issue
tests(3).idark = [3176]; %21:24
...
for i=1:length(tests);
    ...
    diode_per_heat = vis_park.Lat(tests(i).iheat)./vis_park.Lat(tests(i).iheat(1));
    for j=1:length(tests(i).iheat);
        ...
        vis_per_heat(j,:) = vis_per_heat(j,:)./diode_per_heat(j);
        nir_per_heat(j,:) = nir_per_heat(j,:)./diode_per_heat(j);
    end;
