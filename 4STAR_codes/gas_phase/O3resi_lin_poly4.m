function resi=O3resi_lin_poly4(x0,meas,PAR)
...
I_meas  = log(Idat) + o3_coef.*x0(:,1)+ o4_coef.*x0(:,2) + h2o_coef.*x0(:,3) + x0(:,4)*ones(length(Xdat),1) + x0(:,5)*(Xdat) + x0(:,6)*(Xdat).^2 + x0(:,7)*(Xdat).^3 + x0(:,8)*(Xdat).^4;
