function [tau_h2oa tau_h2ob avg_U1] = cwvcorecalc(starsun,modc0,model_atmosphere,cross_sections)
...
for wrange=[1,2,3];
    ...
    %800 nm analysis
        ...
        labresi = 'resi800';
        wln_vis  = wln_vis2;
        wln_ind2 = find(wvis<=wvis(nm_0870)&wvis>=wvis(nm_0780)); 
        cwv_ind1 = find(wvis<=wvis(nm_0870)&wvis>=wvis(nm_0780)); 
    ...
    %700 nm analysis
        ...
        labresi = 'resi700';
        wln_vis  = wln_vis3;
        wln_ind2 = find(wvis<=wvis(nm_0750)&wvis>=wvis(nm_0685)); 
        cwv_ind1 = find(wvis<=wvis(nm_0750)&wvis>=wvis(nm_0685)); 
...
%wln_ind2 = find(wvis<=0.750&wvis>=0.690);
%wln_ind2 = find(wvis<=0.870&wvis>=0.780);
%
qqvis = length(wvis);
qqnir = length(wnir);
pp    = length(starsun.t);
...
% [poly2_,poly2_c_,iter,order,thresh,fn]=backcor(wvis(wln_ind),starsun.tau_aero(goodTime(i),wln_ind));
    if lampcalib==1
        ...
        [poly3_,poly3_c_,iter,order_lin,thresh,fn] = backcor(wvis(wln_ind2),tau_visOD(i,wln_ind2),order_in2,thresh_in2,'atq');% backcor(wavelength,signal,order,threshold,function);
...
%  linkaxes(ax,'x');
 ...
 % slant transmittance
 if lampcalib==1
     Tw=spc_vis_lamp(:,wln_ind2).*exp(tau_aero'.*(repmat(starsun.m_aero_avg,1,length(wvis(wln_ind2))))+...
...
%      std_U1(avg_U1==0)     = NaN;
 ...
 % HITRAN convolved with 4STAR (we might want to create LUT for various alt)
    ...
    x = repmat(starsun.m_H2O_avg,1,length(cwv_ind1)).*(U);
    acoef = repmat((afit_H2O(cwv_ind1,1))',length(starsun.t),1);
    bcoef = repmat((bfit_H2O(cwv_ind1,1))',length(starsun.t),1);
    ...
    figure;
    plot(wvis(wln_ind2),waterOD    (end,:),'-b');hold on;   % measured
    plot(wvis(cwv_ind1),waterODcalc(end,:),'--r');hold on;   % calculated from coef
    ...
    plot(1000*wvis(wln_ind2),waterOD(end-10,:)/20,'--c');
...
visTw=spc_vis_lamp(:,wln_ind2).*exp(tau_aero'.*(repmat(starsun.m_aero_avg,1,length(wvis(wln_ind2))))+...
    (starsun.tau_ray_avg(:,wln_ind2)).*repmat(starsun.m_ray_avg,1,length(wvis(wln_ind2))));
viswvOD = -log(visTw);
% compare c0 with lamp Tw
figure;
plot(wvis(wln_ind2),waterOD(end,:),'-b');hold on;
plot(wvis(wln_ind2),viswvOD(end,:),'--r');
...
for i = 1:length(starsun.t)
       ...
       meas = [wvis(wln_vis)' y'];
       ac = real(afit_H2O(wln_vis,kz)); ac(isNaN(ac)) = 0; ac(ac<0) = 0; ac(isinf(ac)) = 0;
       bc = real(bfit_H2O(wln_vis,kz)); bc(isNaN(bc)) = 0; bc(bc<0) = 0; bc(isinf(bc)) = 0;
       PAR = [ac bc];
       % Set Options
       ...
       ypos = logical(y>=0);
       if ~isNaN(y(1)) && isreal(y) && sum(ypos)>length(wln_vis)-15
            ...
            if isNaN(U_) || ~isreal(U_) || U_<0
               ...
               % plot fitted figure
               yopt_ =  exp(-afit_H2O(wln_vis,kz).*(real(U_)).^bfit_H2O(wln_vis,kz));
               yopt  = -log(yopt_);
               figure(444);
               plot(wvis(wln_vis),y,'-b');hold on;
               plot(wvis(wln_vis),yopt,'--r');hold off;
               ...
               ymax = yopt + 0.2;
               axis([min(wvis(wln_vis)) max(wvis(wln_vis)) 0 max(ymax)]);
               pause(0.0001);
            end
       else
            U_ = NaN;
...
%x0OD    = real(nanmean(U,2)/H2O_conv)./starsun.m_H2O_avg;
% figure;
% plot(cwv_opt700c0,'.b');hold on;
% plot(cwv_opt700lamp,'.g');hold on;
% plot(cwv_opt700c0,'.c');hold on;
% legend('wv c0','wv lamp','wv coef');
%% end fmincon
%--------------------------------------------------------------------------
%--------------------------
 % save cwv into file
