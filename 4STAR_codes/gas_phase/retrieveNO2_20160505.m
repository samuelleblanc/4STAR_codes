...
% NAME:
%   retrieveNO2
%   this is with baseline subtraction and ortho cross sections; new one is
%   simpler
% 
% PURPOSE:
...
% -------------------------------------------------------------------------
%% function routine
function [no2] = retrieveNO2_20160505(s,wstart,wend)
...
% plot residual to fit (ind 12 in wln, which is 459nm).
        figure;
        plot(s.m_aero,1000*spectrum_sub(:,ires),'.','color',[0.8,0.8,0.8],'markersize',8);
...
% basis = [no2coef(wln), no2coefdiff(wln), o4coef(wln), o3coef(wln)];
    ...
    % plot orthonormal cross sections
            figure(10011);
            plot(s.w(wln),20*spectrum_sub(1500,:),'-k');hold on;
    ...
    for k=1:length(s.t);
        %coef=real(basis\spectrum_sub(k,:)');
   ...
   no2Err   = (spectrum_sub'-RR_d(:,:))./repmat((no2_298Kcoef(wln)),1,length(s.t));      
   ...
   no2spectrum     = spectrum_sub-RR_d' + (ccoef_d(1,:)/no2_norm)'*basis(:,1)';
