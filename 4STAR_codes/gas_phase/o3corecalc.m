...
for i = 1:length(starsun.t)
    ...
    %x0 = [0.3 1 0.5 0.75 0.8 -2]; % 2nd order
    x0 = [0.3 1 0.5 0.1 0]; % 1st order
    ...
    PAR  = [o3coef(wln) o4coef(wln)*10000 h2ocoef(wln)*10000];
       ...
       options = optimset('Algorithm','interior-point','LargeScale','off','TolFun',1e-6,'Display','notify-detailed','TolX',1e-6,'MaxFunEvals',1000);
       
       % bounds
%            lb = [0 0 0 -10 -10 -10 -10];
%            ub = [1 5 1 20 20 20 20];%o3-1000;h2o-10000o4-50000
%            lb = [0 0 0 -10 -10 -10];
%            ub = [1 5 1 20 20 20];% 2nd order
             lb = [0.20 0 0 0 -1];
             ub = [0.45 5 0.5 1 0];% 1st order
...
%                    yopt_ =  exp(-(o3coef(wln).*real(U_(1)))).*exp(-(o4coef(wln).*10000*real(U_(2)))).*exp(-(h2ocoef(wln).*10000*real(U_(3))))...
%                             .*exp(-(U_(4) + U_(5)*meas(:,1)));%this was first order with error in meas dependent (lambda)
                     yopt =  (o3coef(wln).*real(U_(1)) + (o4coef(wln).*10000*real(U_(2)))) + (h2ocoef(wln).*10000*real(U_(3)))...
               ...
               % assign fitted spectrum
                ...
                tau_o3o4h2o_subtract(i,wln) = -log(exp(-(o3coef(wln).*real(U_(1)))).*exp(-(o4coef(wln).*real(U_(2)))).*exp(-(h2ocoef(wln).*real(U_(3)))));
                       ...
                       title([datestr(starsun.t(i),'yyyy-mm-dd HH:MM:SS') ' Alt= ' num2str(starsun.Alt(i)) 'm' ' O3= ' num2str(o3_DU_round) '[DU]']);
                       ymax = yopt + 0.2;
