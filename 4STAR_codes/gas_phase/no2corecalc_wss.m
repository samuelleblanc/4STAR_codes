function [ss NO2conc NO2resi no2OD tau_aero_subtract] = no2corecalc_wss(starsun,no2coef,o4coef,o3coef,wln,tau_OD,nm_startpca,nm_endpca)
...
% MS, 2015-04-16  added capability to retrieve with shift/stretch
%----------------------------------------------------------------------
...
%% calculate meas derivative for ss fit
% sft is OD'/OD;
sft = finite_el_der(tau_OD,wln,nm_startpca,nm_endpca);

for i = 1:length(starsun.t)
           
    %x0 = [0.3 1 0.3 0.05 -0.1 0.5]; % this is initial guess (-2 for 6th element -0.1 for 7th element)
    x0 = [0.3 1 0.3 0.05 -0.1 0.8];     % this is initial guess (-2 for 6th element -0.1 for 7th element)
    ...
    PAR  = [no2coef(wln)/1000 o4coef(wln)*100000 o3coef(wln) Ashift];
       % Set Options
...
%            ub = [0.2 5 0.5 2 2 2];%this is with o4 *10000
       ...
       if ~isNaN(y(1)) && isreal(y) && sum(ypos)>length(wln)-15 && sum(isinf(y))==0
            ylarge = logical(y>=5);
            if sum(ylarge)<10
            [U_,fval,exitflag,output]  = fmincon('NO2resi_wss',x0,[],[],[],[],lb,ub, [], options, meas,PAR);
            end
...
%                         sc_residual = [sc_residual;NaN];
                          U_(1) = NaN; U_(2) = NaN; U_(3) = NaN; U_(4) = NaN; U_(5) = NaN; U_(6) = NaN; 
                end
...
%                             .*exp(-(U_(4) + U_(5)*meas(:,1) + U_(6)*meas(:,1).^2));
                   ...
                   yopt  = -log(yopt_);
                   Y_model = ((no2coef(wln))/100).*real(U_(1))+(o4coef(wln).*100000.*real(U_(2)))+(o3coef(wln).*real(U_(3)))...
                              +(U_(4) + U_(5)*meas(:,1)) + U_(6)*Ashift;
...
%                          plot(starsun.w(wln),Y_model,'--r');hold on;
