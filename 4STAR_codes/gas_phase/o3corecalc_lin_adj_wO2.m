function [O3conc,H2Oconc,O4conc,O2conc,O3resi,o3OD,allvar] = o3corecalc_lin_adj_wO2(s,o3coef,o4coef,h2ocoef,o2coef,wln,tau_OD,x0)
...
for i = 1:length(s.t)
    ...
    PAR  = [o3coef(wln) o4coef(wln) h2ocoef(wln) o2coef(wln) s.c0(wln)'];
       ...
       if ~isNaN(y(1)) && isreal(y) && sum(ypos)>length(wln)-15 && sum(isinf(y))==0
            ...
            if sum(signal)<10
                                if isreal(x0(i,:))
                                    ...
                                    [U_,fval,exitflag,output]  = fmincon('O3resi_lin_wO2',x0(i,:),[],[],[],[],lb,ub, [], options, meas,PAR);
                ...
                %                    yfit = o3coef(wln).*U_(1)+ o4coef(wln).*U_(2) + h2ocoef(wln).*U_(3) + o2coef(wln).*U_(4) + U_(5)*ones(length(lm),1) + U_(6)*(lm) + U_(7)*(lm).^2 + U_(8)*(lm).^3;    
                ...
                %                    yo2subtract   =  (o2coef(wln).*real(U_(4)));
                %                    polysubtract  =  U_(5)*ones(length(lm),1) + U_(6)*(lm) + U_(7)*(lm).^2 + U_(8)*(lm).^3;
                %                    yo3subtractall=  yo3subtract + yo4subtract + yh2osubtract + yo2subtract;
                %                
                ...
                %                        plot(s.w(wln),yo2subtract,'--y');hold on;
                ...
                %                        legend('measured','calculated (fit)','o3 spectrum to subtract','o4 spectrum to subtract','h2o spectrum to subtract','o2 spectrum to subtract','aerosol baseline','subtracted spectrum');
                ...
                % 
                %                        ax(2)=subplot(212);
                %                        plot(s.w(wln),ymeas'-yo4subtract-yh2osubtract-yo2subtract-polysubtract,'-b');hold on;% o3 measured
                %                        plot(s.w(wln),yo3subtract,'--r');hold on;                               % o3 retrieved
                %                        plot(s.w(wln),ymeas'-yo4subtract-yh2osubtract-yo2subtract-yo3subtract-polysubtract,':k');hold off;% o3 residual
