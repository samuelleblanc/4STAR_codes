function [c0gases]=starc0gases(t,verbose,gas,mode,instrumentname)
...
% Modified, SL, 2022-10-26, v2.0 Adding instrumentname, for handling of 4STARB
%                           (including 4STARB reference spectra)
%------------------------------------------------------------------------
...
elseif strcmp(instrumentname,'4STARB')
    if t>datenum([2019 8 1 0 0 0]) % use MLO May-2022
        if now>=datenum([2019 8 1 0 0 0])
            if strcmp(gas,'O3')
                if mode==0
                    % use MLO c0
                    error(' 4STARB O3 gas retrievals is not implemented with modified langley method (mode=0), only reference spectra (mode=1)')
                elseif mode==1
                    % use ref_spec
                    tmp = load(which(['4STARB_20220514O3refspec.mat']));
                    c0gases = tmp; %.o3refspec;
                end
            elseif strcmp(gas,'NO2')
                if mode==0
                    % use MLO c0
                    error(' 4STARB NO2 gas retrievals is not implemented with modified langley method (mode=0), only reference spectra (mode=1)')
                elseif mode==1
                    % use ref_spec
                    tmp = load(which(['4STARB_20220514NO2refspec.mat']));
                    c0gases = tmp;%.no2refspec;                  
                end
            ...
            elseif strcmp(gas,'HCOH')
                if mode==0
                    % use lamp FEL?
                    error(' 4STARB HCOH gas retrievals is not implemented with modified langley method (mode=0), only reference spectra (mode=1)')
                elseif mode==1
                    % use ref_spec
                    tmp = load(which(['4STARB_20220511HCOHrefspec.mat']));
                    c0gases = tmp;%.hcohrefspec;
                end
            end
        end
