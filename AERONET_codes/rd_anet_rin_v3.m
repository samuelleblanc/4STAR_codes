function cimel = rd_anet_rin_v3(filename);
% cimel = rd_anet_rin_v3(filename);
%This should read Aeronet v3 refractive indices files.
...
if fid>0
    ...
    if feof(fid)||~startsWith(tmp, 'AERONET Version 3')
        ...
        while ~feof(fid)&&~(startsWith(tmp,'Site,')||startsWith(tmp,'Date,'))
            tmp = fgetl(fid);
        end
...
bins = fieldnames(cimel);
lambdas = []; lambda = []; field = [];
for f = length(bins):-1:1
    if ~(endsWith(bins{f},'nm_')||(endsWith(bins{f},'m_')&&~endsWith(bins{f},'_m_')))
        bins(f) = [];       
    else
        [part1, part2] = strtok(fliplr(bins{f}),'_'); 
        lambda_str = string(fliplr(part1)); 
        field_str = string(fliplr(part2(2:end)));
        lambdas = unique([lambdas,lambda_str ]);
        field = unique([field, field_str]);
        lambda = unique([lambda,sscanf(lambda_str,'%f')]);
    end
end
for f = 1:length(field)
    cimel.(field{f}) = NaN([size(cimel.time,1),length(lambda)]);
end
...
for f = length(bins):-1:1
        [part1, part2] = strtok(fliplr(bins{f}),'_'); 
        lambda_str = string(fliplr(part1)); 
        wl = sscanf(lambda_str,'%f');
        wl_i = find(lambda == wl);
        field_str = string(fliplr(part2(2:end)));
        cimel.(field_str)(:,wl_i) = cimel.(bins{f});
end
