...
s.ground = [datenum(2022,09,02,15,0,0) datenum(2022,09,02,23,0,0)];

%s.flight = [datenum(2022,05,09,15,50,00) datenum(2022,05,09,20,00,00)]; 
...
s.O3h=21; %  
s.O3col=0.2757; %From Mauna Loa Pandora
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=2.9389e-01*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
...
if isfield(s, 'Pst'); 
    s.Pst(find(s.Pst<10))=1013.15; %MLO on 2022-05-08
end;
% other tweaks 
