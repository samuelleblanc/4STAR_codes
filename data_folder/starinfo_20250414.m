...
s.ground = [datenum(2025,04,14,18,50,54) datenum(2025,04,14,22,54,50)];
...
s.aeronet_valid_time = [datenum(2025,04,14,21,43,00) datenum(2025,04,14,22,54,50)];
s.sd_aero_crit=0.008;  
% s.xtra_langleyfilesuffix = 'MLO_May2022_Day9';
             
% Ozone and other gases 
s.O3h=21; %  
s.O3col=0.332; %From SUOMI NPP / OMPS over Ames
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=0.132 % from TEMPO tropospheric NO2 in DU %7.134e-2*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
...
if isfield(s, 'Pst') 
    %s = interpol_MetNav(s,[getnamedpath('stardat'),s.NavMetfile]);
    s.Pst(find(s.Pst<10))=1016; %for Marina Airport
end
% other tweaks 
