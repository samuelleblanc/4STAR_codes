...
s.ground = [datenum(2025,02,25,20,10,47) datenum(2025,02,25,23,06,47)];
...
s.O3h=21; %  
s.O3col=0.307; %From SUOMI NPP / OMPS over Ames
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=0.192 % from TEMPO tropospheric NO2 in DU %7.134e-2*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
...
if isfield(s, 'Pst') 
    %s = interpol_MetNav(s,[getnamedpath('stardat'),s.NavMetfile]);
    s.Pst(find(s.Pst<10))=1020; %for Marina Airport
end
% other tweaks 
