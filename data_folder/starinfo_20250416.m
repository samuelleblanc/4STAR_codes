...
s.ground = [datenum(2025,04,16,21,01,46) datenum(2025,04,17,01,11,09)];
%s.flight = [datenum(2024,10,30,18,16,28) datenum(2024,10,30,19,42,36)];
s.loose_aeronet_comparison = true;
s.langley1 = [datenum(2025,04,16,22,51,46) datenum(2025,04,17,00,35,09)];
...
s.O3h=21; %  
s.O3col=0.347; %From SUOMI NPP / OMPS over Ames
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=0.072 % from TEMPO tropospheric NO2 in DU %7.134e-2*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
...
if isfield(s, 'Pst') 
    %s = interpol_MetNav(s,[getnamedpath('stardat'),s.NavMetfile]);
    s.Pst(find(s.Pst<10))=1014; %for Marina Airport
end
% other tweaks 
