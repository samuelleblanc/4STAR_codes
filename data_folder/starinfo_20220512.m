...
s.langley1 = [datenum(2022,05,12,16,03,00) datenum(2022,05,12,18,21,0)];
...
s.sd_aero_crit=0.01;  
s.xtra_langleyfilesuffix = 'MLO_May2022_Day6';
             
% Ozone and other gases 
s.O3h=21; %  
s.O3col=0.2736; %From Mauna Loa Pandora
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=5.883e-2*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
% s.dirty = [datenum(2018,10,06,7,35,0) datenum(2018,10,06,7,45,0)];
% s.clean = [datenum(2018,10,06,7,50,49) datenum(2018,10,06,7,58,56)];
s.ground = [datenum(2022,05,12,15,12,00) datenum(2022,05,12,19,10,00)];
...
if isfield(s, 'Pst'); 
    s.Pst(find(s.Pst<10))=680.44; %MLO on 2022-05-08
end;
% other tweaks 
