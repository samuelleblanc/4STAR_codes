...
s.ground = [datenum(2025,04,09,18,32,10) datenum(2025,04,09,22,46,30)];
...
s.O3h=21; %  
s.O3col=0.295; %From SUOMI NPP / OMPS over Ames
Loschmidt= 2.686763e19; %molecules/cm2
s.NO2col=0.083 % from TEMPO tropospheric NO2 in DU %7.134e-2*(Loschmidt/1000); %  5.883e-2 DU from Mauna Loa Pandora
...
if isfield(s, 'Pst') 
    %s = interpol_MetNav(s,[getnamedpath('stardat'),s.NavMetfile]);
    s.Pst(find(s.Pst<10))=1022; %for Marina Airport
end
% other tweaks 
